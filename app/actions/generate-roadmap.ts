...
Next, evaluate whether the input has coherent meaning. Treat meaningless letter sequences, keyboard mashing such as "asdfg" or "xyz", placeholder repetition such as "test test", and text with no understandable learning goal as gibberish. Do not generate content for it. Return exactly: {"isValidTopic":false,"isPolicyViolation":false,"violationReason":null,"isGibberish":true,"message":"","roadmap":null,"careerInsights":null}.
    ...
    async function requestGroqRoadmap(client: Groq, provider: string) {
      ...
      try {
        ...
        if (choice.finish_reason === "length") {
          throw new IncompleteRoadmapGenerationError(
            `${provider} truncated the ${isAdvanced ? "advanced" : "standard"} roadmap at the completion-token limit.`,
          );
        }
