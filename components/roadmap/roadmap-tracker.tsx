...
export function RoadmapTracker({ roadmap }: Props) {
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-3xl border border-black/8 bg-white shadow-[0_20px_70px_rgba(23,33,27,.08)] sm:mt-10 md:rounded-[2rem]">
      <header className="grid gap-5 border-b border-black/8 p-4 sm:p-6 md:grid-cols-[1fr_auto] md:gap-8 md:p-8 lg:p-10">
        <div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.16em] text-[#3c7156] sm:text-xs sm:tracking-[.2em]"><BadgeCheck size={16} /> Your personal path</div><h2 className="mt-3 break-words text-2xl font-black tracking-[-.04em] sm:text-3xl md:text-4xl">{roadmap.title}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-black/50 sm:text-base sm:leading-7">{roadmap.description}</p></div>
      ...
      <div className="p-4 sm:p-6 md:p-8 lg:p-10">
        <div className="mb-5 flex items-center gap-2 text-sm font-semibold text-black/45 sm:mb-7"><Clock3 className="shrink-0" size={16} /> Estimated path: {roadmap.estimatedDuration}</div>
        <ol className="relative space-y-3 before:absolute before:bottom-7 before:left-[1.1rem] before:top-7 before:w-px before:bg-black/10 sm:space-y-4 sm:before:left-[1.35rem]">
          {milestones.map((item, index) => (
            <motion.li initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .05 }} key={item.id} className={cn("relative flex gap-3 rounded-2xl border p-3 transition sm:gap-5 sm:p-5", item.isCompleted ? "border-[#3c7156]/15 bg-[#f5faee]" : "border-black/8 bg-white hover:border-black/15")}>
              <button type="button" disabled={isPending} onClick={() => handleToggle(item)} aria-label={`${item.isCompleted ? "Mark incomplete" : "Complete"} ${item.title}`} className={cn("relative z-10 grid size-9 shrink-0 place-items-center rounded-full border-2 transition sm:size-11", item.isCompleted ? "border-[#3c7156] bg-[#3c7156] text-white" : "border-black/15 bg-white text-black/25 hover:border-[#3c7156]")}>{item.isCompleted ? <Check size={18} strokeWidth={3} /> : <Circle size={12} fill="currentColor" />}</button>
