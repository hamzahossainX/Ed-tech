...
type AuthCardProps = { mode: "login" | "register" };
...
export function AuthCard({ mode }: AuthCardProps) {
  ...
  async function register(values: RegistrationInput) {
    ...
    if (!result.success) {
      ...
      if (result.fields) for (const [field, message] of Object.entries(result.fields)) registerForm.setError(field as keyof RegistrationInput, { message });
      setPending(false);
      return;
    }
    const loginResult = await signIn("credentials", { email: values.email, password: values.password, redirect: false });
