...
type AuthCardProps = {
  ...
  /** False when the deployment has no GitHub OAuth credentials configured. */
  isGithubEnabled?: boolean;
};

export function AuthCard({ mode, initialError, initialNotice, isGithubEnabled = true }: AuthCardProps) {
    ...
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden p-4">
      ...
      <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 140, damping: 20 }} className={`relative w-full overflow-hidden rounded-3xl border border-black/8 bg-white/10 p-5 shadow-[0_24px_80px_rgba(23,33,27,.13)] backdrop-blur-md dark:border-white/10 dark:bg-slate-900/50 dark:shadow-[0_24px_80px_rgba(0,0,0,.45)] sm:p-6 ${isRegister ? "max-w-lg" : "max-w-md"}`}>
        ...
        <div className="relative">
          ...
          <p className="mt-1.5 text-xs leading-5 text-black/50 dark:text-white/55">{isRegister ? (isGithubEnabled ? "Create your LearnX profile with email or GitHub." : "Create your LearnX profile with your email.") : "Sign in to continue your learning journey."}</p>
