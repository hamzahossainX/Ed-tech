...
type LandingExperienceProps = { isAuthenticated: boolean };

export function LandingExperience({ isAuthenticated }: LandingExperienceProps) {
  ...
  return (
    <main className="min-h-screen overflow-hidden">
      <section className="relative mx-auto flex min-h-[72vh] max-w-7xl flex-col items-center justify-center px-4 pb-14 pt-8 text-center sm:min-h-[78vh] sm:px-6 sm:pb-20 sm:pt-12 lg:px-10">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .18 }} className="mt-6 max-w-2xl text-base leading-7 text-black/50 dark:text-white/55 sm:mt-8 sm:text-lg sm:leading-8 md:text-xl">Tell us what you want to learn. LearnX builds a personalized, milestone-by-milestone plan in seconds—sign in free and start right away.</motion.p>
        <motion.button initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .26 }} onClick={begin} className="mt-8 flex min-h-12 items-center gap-3 rounded-full bg-[#c8ff65] px-6 py-3 text-base font-black text-slate-950 shadow-[4px_4px_0_#17211b] transition hover:translate-x-1 hover:translate-y-1 hover:text-slate-950 hover:shadow-none dark:text-slate-950 dark:hover:text-slate-950 sm:mt-10 sm:px-8 sm:py-5 sm:text-lg">Start learning <ArrowDown size={19} /></motion.button>
      ...
      <AnimatePresence>
        {started && <motion.section id="start" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-5xl scroll-mt-8 px-4 pb-20 sm:scroll-mt-12 sm:px-6 sm:pb-28 lg:px-10"><RoadmapPrompt isAuthenticated={isAuthenticated} /></motion.section>}
      </AnimatePresence>
    </main>
  );
}
